# 🧪 Ejercicio 119/200 — Explicabilidad con LIME en un modelo de detección de noticias falsas
...
# Configurar estilo gráfico con Seaborn
sns.set_theme(style="whitegrid", palette="pastel")

# 2. 📰 Crear un pequeño dataset de noticias reales (0) y falsas (1)
data = {
    "text": [
...
plt.figure(figsize=(10, 5))
sns.lineplot(data=history_df[["accuracy", "val_accuracy"]], linewidth=2.5)
...
# 11. 🧠 Función que convierte un texto a probabilidad para LIME
def predict_prob(texts):
    texts = tf.convert_to_tensor(texts)  # Convierte lista de textos a tensores
    preds = model.predict(texts, verbose=0)
    return np.hstack([1 - preds, preds])  # LIME espera: [Prob clase 0, Prob clase 1]


# 12. 🧪 Explicabilidad con LIME
class_names = ["Real", "Fake"]
explainer = LimeTextExplainer(class_names=class_names)

# Seleccionamos una noticia para explicar
i = 0
text_to_explain = X_test.iloc[i]
print("📰 Noticia a explicar:", text_to_explain)

# Generamos la explicación local con LIME
exp = explainer.explain_instance(
    text_instance=text_to_explain,
    classifier_fn=predict_prob,
    num_features=6,  # Número de palabras importantes a mostrar
    top_labels=1,  # Solo explicar la clase más probable
)
